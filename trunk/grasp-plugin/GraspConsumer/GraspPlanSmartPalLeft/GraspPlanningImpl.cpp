...
double GraspPlanning_impl::avoidAngleLimit() {
	...
	for (int i = 0; i < arm_path->numJoints(); i++){
		double edist =  weight[i]*(arm_path->joint(i)->q - (arm_path->joint(i)->ulimit + arm_path->joint(i)->llimit) / 2.0); // * 6.28 / (arm_path->joint(i)->ulimit - arm_path->joint(i)->llimit);
		dist +=  edist*edist;
	}
...
bool GraspPlanning_impl::SortGraspPosition(int ObjId, Vector3 objVisPos, Matrix33 objVisRot) {
	...
	while(fin_pos){
		...
		cout << Vector3(trans(rpr)*rpp) << endl;
